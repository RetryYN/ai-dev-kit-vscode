...
本プロジェクトの現行ゴールは **HELIX v2（PM型 TDDベース 生体モデルワークシステム開発ハーネス）の開発工程**。6 Phase の連鎖・`forward_return`・貫通要件・既知の片肺検出事項の正本は [V2 実装計画 Process](docs/plans/process/process-2026-06-03-v2-implementation-roadmap.md)（ここには重複させずリンクで繋ぐ）。

- **スコープ**: 機能要件は **既存の全洗い出しを漏れなく内包**する（`cli/helix-*` 118本・hooks・`cli/lib` Python・SQLite・548機能の既存実態すべて）。**roadmap 全体の監査スコープは全 L・全 doc・全成果物＝「すべて」**（一部追補ではない）。ただし**各実作業の編集範囲は Action Plan / allowed_files / handover / task-plan に従う**（広域スコープは個別作業の allowed_files 制約を弱めない）。
- **最優先 = Phase 1-2 を「ドキュメント ↔ ワークフロー/ドライブ」の循環で回し切る**。ドキュメントカバレッジと駆動機構が成立しない限り HELIX のコアは回らない（`HELIX_CORE.md §0`）。上流設計の本格見直しは循環が回った後段。
- **貫通要件**: Forward V モデル + 駆動モデルは、全 Phase をフル run しても耐える設計であること（＝コードの redesign でなく HELIX プロセス機構の堅牢化が本計画の実体）。
- **Python 化は機能修正＝L6 視座**であり独立 redesign に昇格させない（Phase 3 に内包）。**DB 拡張は自動化が回ってから**（Phase 4、DB ならではの解法が見えてから schema 確定）。

| Phase | 範囲 | 主眼 |
|---|---|---|
| 1 | L0 確認 → L1 / L3 + 対ペア | L1↔L14 運用 / L3↔L12 受入 テスト設計 |
| 2 | L4-L6 + 対ペア | L4↔L9 総合 / L5↔L8 結合 / L6↔L7 単体（片肺解消） |
| 3 | L7 実走 + 自動化 + Python化 本体改修 | Python化=機能修正(L6 視座)でここに内包 |
| 4 | HELIX DB 機能拡張 | 自動化が回ってから（推測 schema を避ける） |
| 5 | L8- テスト実走 + トラブルシュート | 実装・設計の強化 |
| 6 | テストプロジェクト透過フル run | 全体機能のボトルネック改善・強化 |
