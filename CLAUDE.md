...
**検証は Forward V-model に内在する検証サイクル＝ゲートとして機能させる**（ロードマップの Phase として常時追いかけない）。各 L-pair の凍結/前進は、その**検証閉合をゲートで通す**ことで成立する。

- **ゲート定義**: `pair_closure = design + test_design + test_code_anchor + test_execution_pass + trace_symmetry + semantic_gate`。
- **L-pair ゲート**: L6↔L7 = G6（設計/単体テスト設計 freeze）+ **G7（UT 実装 anchor + 実行 pass）**／ L5↔L8 = G8 ／ L4↔L9 = G9（同構造）。
- **正本配置**: 原則 → [HELIX-process-L0-L14](HELIX-workflows/HELIX-process-L0-L14.md)／判定式・evidence schema → [verification-strategy](docs/v2/L1-requirements/helix-workflows-verification-strategy.md)／機械 gate 配線 → [automation-gate-map](HELIX-workflows/helix-process/automation-gate-map.md)。
- **段階導入**: ① L7 anchor closure（UT-ID をテストコードに anchor + 実行 pass 証跡。trace_symmetry だけでは閉じない＝別 subcheck が要る）→ ② detector allowlist（green のみ fail-close 昇格）→ ③ CI enforce。**pair trace strict（現 fail = critical 残）は per-pair opt-in、即 CI 昇格しない**（CI 即 red 回避）。
