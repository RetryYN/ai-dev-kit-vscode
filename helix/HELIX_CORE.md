...
HELIX は、この制御を**人間チームでなく AI 自身がオーケストレーションして自走する**ことを前提に設計する。チーム駆動の harness（例: UT-TDD Agent Harness）が人間の役割分担で判断を担保するのに対し、HELIX は判断を gate / DB / V モデル収束に**機械化**している。ゆえに AI は、その機械的ガードレールの内側で、人間の逐次承認を待たずに安全に自走できる——**ガードレールが安全を保持し、AI が前進を担う**。

この原則の運用は次の通り:

- **AI は自らオーケストレーションして工程を前進させる**。実装・検証・委譲・並列投入・次工程選択を、人間に逐一「進めてよいか」を問わずに駆動する。判断材料は gate / DB / V モデルの現在地から取る。
- **停止条件は機械が定める**。ガードレール（gate 赤 / detector 検出 / V モデル逸脱 / 絶対原則違反）に当たったときのみ止まり、Reverse / Recovery / Incident 等へルーティングする。緑である限り止まらない。
- **人間の明示承認は、不可逆・破壊的境界にのみ留保する**。runtime rules §10（本番影響 / 認証 / 認可 / 決済 / PII / secret / license / schema migration / destructive data operation / 外部 API・infra 変更 / handover・PLAN 矛盾）に該当する操作だけは、AI が自走で確定せず人間に escalate する。それ以外の安全な前進は AI が主導する。
- **「安全」は人間の監視でなく仕組みが保証する**。だからこそ AI 自走が成立する。ガードレールを弱めて自走させること（gate 迂回・検証省略・破壊境界の自己承認）は、この原則の悪用であり禁止する。

## 7. 実行効率化層

skill、command、subagent、Codex / Claude harness で実作業を高速化・分担する。

AI は制約内で**自らオーケストレーションして自走する（§6 AI 主導・安全自走の原則）**が、品質判断の正本は gate / DB / V モデルにあり、AI はそれを代替しない。自走（実行の主導）と判断の機械化（品質の主語を AI に移さない）は両立する。実行効率化層は、HELIX の工程・DB・gate に従わせたまま、AI 主導の前進を高速化・分担するための実行基盤である。
