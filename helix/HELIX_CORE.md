...
これにより、ドキュメント（左腕）・実装（底）・テスト / 検証（右腕）・カバレッジを対応づけ、ずらさず正本化する。設計⇔検証の対は同時に凍結し、片方だけの成立（片肺）を完了扱いにしない。**対は量だけでなく粒度でも対称に閉じる**（設計は対の検証と同じ粒度で書く。機能設計 L6 は単体テスト粒度）。粒度別の具体は `HELIX-workflows/HELIX-process-L0-L14.md` の粒度ペアリング原則を正本とする。
