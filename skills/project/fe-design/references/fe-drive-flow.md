...
> 目的: fe / fullstack 駆動採用時の L2〜L10 における具体的ステップ、モック駆動の実施手順、TL による API 契約導出プロセス、モック由来 debt のライフサイクルを定義する。
>
> 適用範囲: `helix size --drive fe` または `--drive fullstack`（自動判定含む）
>
> 正本: SKILL_MAP.md §drive 別 L2〜L14 / gate-policy.md G2/G3/G4/G5/G6/G7/G10
...
- `helix gate` が `.helix/phase.yaml` の `sprint.drive` が `fe|fullstack` なら以下を debt-register に auto-enqueue:
  ...
  - `MOCK-HARDCODE`（grep 対象、target_sprint=G7、owner=SE）
  - `MOCK-CODE-LEAK`（AST 対象、target_sprint=G7、owner=SE）
...
| ロール | 担当 | L2 | L3 | L6 | L7 | L10 |
|-------|------|----|----|----|----|-----|
| TL | リード | **モック + state-events** | 契約導出 | MOCK-DERIVED-CONTRACT レビュー | .1a (Props 分解) | — |
| Codex 実装 role | 実装 | — | — | — | .1b / .2 / .3 | リファイン |
| QA | テスト / a11y | — | テスト設計 | 単体テスト設計観点 | .4 監査 | 監査 |
...
| Opus (PM) | 統合・判断 | UX 承認参加 | 工程表承認 | L6 境界確認 | Sprint 監視 | Visual 判断 |
...
| MOCK-HARDCODE | G2 通過時 (auto) | G7 | grep でハードコード値が残存していないことを確認 → resolve | **G7 fail-close** |
| MOCK-CODE-LEAK | G2 通過時 (auto) | G7 | `.helix/mock/` が `src/` から import されていないことを AST で確認 → resolve | **G7 fail-close** |
...
| モックのコードを本実装にコピペ | L7.1b で実装フローが「分解 → 移植」する。.helix/mock/ は throw-away |
| state-events.md を書かずに API 契約を書く | TL が L3 で state-events.md 読めないと契約導出不可 |
| モック時のハードコード値（URL / トークン / テストデータ）が本番に残る | G7 で MOCK-HARDCODE が fail-close |
| `import from "../../.helix/mock/..."` | G7 で MOCK-CODE-LEAK が fail-close |
...
- drive 別 L2〜L14 テーブル: `skills/SKILL_MAP.md §drive 別 L2〜L14`
