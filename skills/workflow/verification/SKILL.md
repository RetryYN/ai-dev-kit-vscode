...
- PoC 要否（不確実性が高い領域の有無）
- 事前調査要否（技術・法務・運用）
- 運用テスト設計の初期観点（監視・復旧・運用改善）がある

### L2（画面要求 / 画面設計 / フロントUI + ワイヤーモック）

- UI / 画面が存在する場合、画面要求・ワイヤーモック・導線が対応している
- UI absent の場合、明示 waiver と unskip 条件がある
- 画面要求と後続 API / 状態 / DB 境界の対応が説明されている
- アクセシビリティ・フロントUX磨き上げに渡す観点がある

### L3（要件定義 + 受入テスト設計）

- 要件ID・受入条件・非機能要件が明示されている
- 受入テスト設計が要件IDへ双方向 trace されている
- API / DB / 契約の凍結要否と、後続 L4/L5/L6 への分解方針がある
- 曖昧要件・保留・人間承認事項が carry として分離されている

### L4（基本設計 / 外部設計 + 総合テスト設計）

- 外部インターフェース、主要コンポーネント、責務境界が定義されている
- 総合テスト設計が L4 の外部設計観点へ trace されている
- セキュリティ / 性能 / 運用の大枠リスクがある
- 実装に進めない設計ギャップが carry / add-feature / interrupt に分離されている

### L5（詳細設計 / 内部設計 + 結合テスト設計）

- 内部状態、関数責務、データ変換、エラー処理が定義されている
- 結合テスト設計が L5 の内部連携・状態遷移へ trace されている
- DB / API / 状態 / 依存の整合が説明されている
- 破壊的変更、migration、認証認可、PII、外部APIは承認境界が分離されている

### L6（機能設計 / 仕様書 + 単体テスト設計）

- 関数 / 機能単位の signature、requires、ensures、invariant がある
- 単体テスト設計観点が L6 仕様へ trace されている
- L7 実装・単体テスト実装・単体テスト実施・coverage closure と混同していない
- L7 が未承認の場合、L6 内 `*-UT-CAND-*` または add-feature に分離されている

### L7（実装 + 単体テスト実装 / 実施 / coverage closure）

- L6 仕様と単体テスト設計に対して、テスト実装 → 本体実装の順序を守る
- 単体テスト実施と coverage closure の証跡がある
- 3点レビュー（設計・実装・テスト）が閉じている
- 未承認の L7 作業は add-feature / PLAN 承認へ戻す

### L8（結合テスト）

- L5 結合テスト設計に対する実行証跡がある
- 連携境界、状態遷移、エラー伝播の検証が閉じている

### L9（総合テスト）

- L4 総合テスト設計に対する実行証跡がある
- 主要ユーザーフロー、非機能、運用前提の総合検証が閉じている

### L10（フロントUX / 業務デザイン磨き上げ）

- L2 の画面要求 / wire / UI 方針に対して UX・業務導線の磨き上げ証跡がある
- UI absent waiver がある場合、unskip 条件に抵触していない

### L11（総合レビュー / ユーザー検証 / 要件巻き取り）

- ユーザー検証結果、要件巻き取り、差戻し判断が記録されている
- 新規要件は L1/L3/L4/L5/L6 へ戻すか add-feature に分離されている

### L12（受入テスト）

- L3 受入テスト設計に対する実行証跡がある
- 受入 / 差戻し / 残余リスク受容が明示されている

### L13（運用検証 / 運用テスト）

- 運用手順、監視、復旧、ロールバック、Runbook の実行確認がある
- 本番影響・外部連携・認証・PII は承認済みである

### L14（運用学習 / 運用改善）

- 運用結果、トラブル検出、改善フィードバック、再発防止が記録されている
- HELIX DB / feedback loop へ登録され、次の PLAN / PR candidate へ接続できる
...
※ V-L3〜V-L6 は本スキル固有の検証レイヤー番号。
  HELIX 工程番号は L0-L14 を正とし、L4=基本設計/外部設計、L5=詳細設計/内部設計、L6=機能設計/仕様書、L7=実装+単体テスト closure として扱う。
```
...
## 8.5. フェーズ証跡 vs 対応右腕実行

| 検証項目 | 証跡作成フェーズ | 対応実行フェーズ | 備考 |
|---------|----------------|-----------------|------|
| 要求・運用テスト設計 | L1（G1 通過時） | L14 | 運用学習 / 改善 feedback で閉じる |
| 画面要求・ワイヤーモック | L2（G2 通過時） | L10 | UI absent の場合は waiver と unskip 条件で管理 |
| 要件・受入テスト設計 | L3（G3 通過時） | L12 | 受入テスト実行で要件充足を確認 |
| 基本設計・総合テスト設計 | L4（G4 通過時） | L9 | 外部設計・主要フロー・非機能を総合テストで確認 |
| 詳細設計・結合テスト設計 | L5（G5 通過時） | L8 | 内部連携・状態遷移・エラー伝播を結合テストで確認 |
| 機能設計・単体テスト設計 | L6（G6 通過時） | L7 | 単体テスト実装 / 実施 / coverage closure で確認 |
| 実装品質 | L7（G7 通過時） | L7 | 3点レビューと単体テスト closure を確認 |
| 運用検証 | L13 | L13/L14 | Runbook / 監視 / rollback / feedback loop を確認 |

L6 は機能設計 / 仕様書と単体テスト設計を凍結する工程であり、L7 の実装・単体テスト実施・coverage closure を代替しない。
...
L2    画面要求 / UI / ワイヤーモック ←──────→ L10 フロントUX / 業務デザイン磨き上げ
L3      要件定義 + 受入テスト設計    ←────→ L12 受入テスト
L4        基本設計 / 外部設計 + 総合テスト設計 ←→ L9 総合テスト
L5          詳細設計 / 内部設計 + 結合テスト設計 ←→ L8 結合テスト
L6            機能設計 / 仕様書 + 単体テスト設計 ←→ L7 実装 + 単体テスト closure
                         L11 総合レビュー / ユーザー検証 / 要件巻き取り
```
...
L12 受入テスト時に `D-*` 成果物と実装コードを自動突合し、仕様差分を検出する。
