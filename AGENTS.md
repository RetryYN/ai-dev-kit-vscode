...
- `helix/HELIX_RUNTIME_RULES.md` — Claude / Codex 共通の実行規律
- `skills/SKILL_MAP.md` — フロー・ゲート・スキル一覧
- `helix/CODEX_RUNTIME_ADAPTER.md` — Codex CLI 固有の runtime 差分
...
2. `helix/HELIX_RUNTIME_RULES.md` が存在するか確認する。
3. `skills/SKILL_MAP.md` が存在するか確認する。
4. `helix/CODEX_RUNTIME_ADAPTER.md` が存在するか確認する。
5. `.helix/handover/CURRENT.json` が存在する場合は `helix handover status --json` を実行する。
6. handover が stale なら作業を止め、stale reason をユーザーに伝える。
7. stale でなければ `helix handover update --owner codex` で所有権を移し、`.helix/handover/CURRENT.md` の Next Action に従う。
8. handover がなければ通常開始し、「OK: セッション初期化完了」と宣言する。

## Codex Runtime Adapter

Codex CLI は独自 workflow mode ではない。HELIX Core と Runtime Rules に従い、Codex 固有の差分だけ `helix/CODEX_RUNTIME_ADAPTER.md` で扱う。

- 設計、技術的難易度評価、実装、レビュー、テスト、検証は HELIX の PLAN / workflow / gate に従って進める。
...
- `helix codex` で呼ぶ **委譲 Codex** は `git add` / `git commit` / `git push` を一切しない（CODEX_DISCIPLINE_PROMPT §5 で強制）。コミット判断は呼び出し元またはユーザー (Opus / PM) が行う。委譲先が勝手に commit した場合は `git reset --soft HEAD~1` で巻き戻して呼び出し元で commit し直す。
...
### HELIX-workflows 参照原則

**工程・workflow 判断時はまず [HELIX-workflows/](HELIX-workflows/) を正本として参照する**。L0-L14 の工程定義は [HELIX-workflows/HELIX-process-L0-L14.md](HELIX-workflows/HELIX-process-L0-L14.md)、各工程詳細・workflow 別文書は [HELIX-workflows/helix-process/](HELIX-workflows/helix-process/) (L0-L14 工程 doc / workflow doc / 工程専門 / 管理・自動化基盤 doc 群、integration-map / asset-mapping / folder-structure-review / detection-routing / layer-context-injection / cross-cutting-mechanisms / automation-gate-map / continuous-run-context-management / fe-detector-spec / observability-metrics 等を含む)。

`AGENTS.md` / `helix/HELIX_CORE.md` / `helix/HELIX_RUNTIME_RULES.md` / `skills/SKILL_MAP.md` / runtime adapter は HELIX-workflows を正本として参照する形で同期する。drift 発見時は HELIX-workflows と HELIX Core を正、本ファイル群を retrofit する方針。

新規 PLAN 起票・workflow 判断・コマンド整備など、企画・設計判断に入る前に [HELIX-workflows/helix-process/integration-map.md](HELIX-workflows/helix-process/integration-map.md) §結論と優先順位 を必ず確認する (整理済み企画書を読まずに自前 carry list を並べないこと)。
...
以下 mode は workflow doc 正本 + PLAN kind / template で扱う。**`helix refactor` / `helix retrofit` / `helix recovery` 等の CLI は存在しない**ため絶対に叩かない:
...
全 workflow で作業前に合格基準・検証条件を置く。実装を伴う場合は「テスト → 実装」順序を厳守し、テストアフター禁止。Discovery では仮説 / PoC / 検証条件 / 採用・棄却基準を先に置く。Refactor では変更前の保護網テスト存在を前提。詳細は [HELIX-workflows/HELIX-process-L0-L14.md](HELIX-workflows/HELIX-process-L0-L14.md)、`helix/HELIX_CORE.md`、`helix/HELIX_RUNTIME_RULES.md` を参照。

### 特殊 workflow / HELIX W (AI エージェントシステム時のみ)

[two-stage-agent-design.md](HELIX-workflows/helix-process/two-stage-agent-design.md) — 通常 workflow と並列ではなく特殊 workflow として扱う。
...
実行担当は自分で全作業を抱え込まず、工程表の role と作業種別に従って `helix codex`、`helix claude --dry-run`、`helix team`、利用可能なサブエージェントを使う。使えない場合は理由を final の evidence に残す。
