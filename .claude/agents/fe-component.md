...
## エラー表示の原則
- ユーザーに見せるエラーメッセージは「何が起きたか」「何をすべきか」のみ
- 技術的詳細（スタックトレース、エラーコード内部名、DB エラー）は表示しない
- 開発モードでのみ詳細表示（NODE_ENV === 'development'）
- エラー境界（Error Boundary）で未処理例外をキャッチし、フレンドリーな画面を表示
- API エラーレスポンスの内部フィールドを UI に直接バインドしない
