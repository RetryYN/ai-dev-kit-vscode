...
| 2026-06-15 | **follow-up Action = DF-FCCI-CI-RATCHET-PUSH 解消**（ユーザー goal「両方片づけて」+ AskUserQuestion で「CI-RATCHET-PUSH のみ」確定 / TL tl-advisor 諮問 passed = 同 scope 限定推奨）。ci.yml `detector-gate` に push event 用 export step（`if: github.event_name == 'push'`、`${{ github.event.before }}..${{ github.sha }}` diff → HELIX_CHANGED_FILES）を追加し push:[main] CI の ratchet fail-OPEN を解消。TDD: 契約 test `test_ci_workflow_pins_detector_gate_contract` に push assertion 追加（red→green、test 件数 88 維持・新規関数なし）+ bats ミラー sync。§10 の DF-FCCI-CI-RATCHET-PUSH を解消済みへ更新。**PM 検証 = contract 88 / l0-l14 bats 57 green、ci.yml YAML parse OK**。残 PARKED（W17/W18 full-required / W2 strictflow / ruff advisory / branch protection 人間作業）は不変。 | PM (Opus) + tl-advisor |
...
- **DF-FCCI-BRANCHPROT**: GitHub branch protection への Required check 登録（repo 設定・別権限の**人間作業**。`gh` CLI 不在 + escalation 該当=admin 権限/credential/本番保護設定のため、PM/Codex は API 迂回せず手順提示のみ）。帰属 = handover の人間向け Next Action。**登録手順（GitHub Web UI 推奨・既存設定を壊さない）**:
  1. `https://github.com/RetryYN/ai-dev-kit-vscode/settings/branches` → Branch protection rules → `main`（rule 無ければ Add rule、Branch name pattern = `main`）。
  2. **Require status checks to pass before merging** を ON。
  3. 検索窓で **`test`** と **`detector-gate`** を選択（= ci.yml の 2 job 名）。⚠️ **`detector-gate` は新規 job のため、最低 1 回 CI 実行済み**（PR 起票 or push）でないと候補に出ない。出ない場合は先に CI を 1 回走らせる。
  4. （推奨）**Require branches to be up to date before merging** を ON（古い base での merge を防止）。
  5. （`main` は `Require a pull request before merging` も維持 = 既存 policy「main は PR-gated」）。Save changes。
  - `gh` CLI 利用可時の参考（ユーザー実行・既存 protection を**全置換**するため UI 推奨）: `gh api -X PATCH repos/RetryYN/ai-dev-kit-vscode/branches/main/protection/required_status_checks -F strict=true -f 'contexts[]=test' -f 'contexts[]=detector-gate'`（required_status_checks が既存の場合の差分更新）。
  - 完了確認: PR で `test` / `detector-gate` 両方が Required（merge blocking）として表示されること。
- **DF-FCCI-CI-RATCHET-PUSH**: ~~CI `detector-gate` の ratchet detector（4件）は `push:[main]` event で vacuous~~ → **2026-06-15 解消（follow-up Action）**。原因 = push event に HELIX_CHANGED_FILES export step が無く [PR-only]、`changed_files()` が `source_status=unavailable` → `_ratchet_required_clean` が clean=True に override = fail-OPEN。**対処** = `ci.yml` の `detector-gate` job に push event 用 export step（`if: github.event_name == 'push'`、`fetch-depth: 0` で到達可能な `${{ github.event.before }}..${{ github.sha }}` の diff range を HELIX_CHANGED_FILES へ注入）を追加。新ブランチ/force-push 等で before が全0/未到達のときは `|| true` で空文字 export → `source_status=available_empty`（changed-file 0 件 = clean。git diff 失敗を「変更なし」扱いにする稀な edge = P2 残リスク、force-push は branch protection で抑止）。PR path + `helix push --gate` が依然 authoritative。pin = 契約 test `test_ci_workflow_pins_detector_gate_contract` に push step assertion 追加（test 件数 88 維持・新規 test 関数なし）+ bats ミラー sync。検証 = pytest 88 / l0-l14 bats 57 green、TDD red→green 実証。帰属 = weakness-map W4 + L6↔L7 G7 pending evidence。元の TL P0 諮問（受容裁定）は当時の判断として履歴保持。
