...
| C8 (new) | **superseded by L7-route-engine-c8-integration-testplan**: C' (`L7-route-engine-drift-type-retrofit-extplan`) 完遂後 carry だった route → `helix plan draft --kind retrofit` 統一 (ADR-042 採用) の E2E テストは `cli/tests/test-route-engine-c8-integration.bats` で実装済。`helix route suggest --signal dependency_outdated --json` の `recommended_command` contract と drift_type 7 種の bats 回帰を追加し、`helix retrofit init` は PLAN 確定後の手動着手として位置付けを維持 | **completed** | completed by L7-route-engine-c8-integration-testplan |
