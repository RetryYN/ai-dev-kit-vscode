...
- [ ] **デグレ判定 (重複追加検出)**: 新規追加 (FR / ADR / PLAN / 設計内容) が既存 doc に既にカバーされている内容と重複していないか機械検出する。今 session で「FR-07 に書いてあるのに ADR-047 で重複作成」「L4 PLAN が generates する L9 doc があるのに L9 PLAN 過剰起票」を計 3 度繰り返した = デグレの典型 (既存資産を見ずに重複生成)。skill-radar / code catalog の類似度検出を要求・設計層 (FR / ADR / 設計 doc) へ拡張し、新規起票前に「既存に同等カバーがないか」を warn する gate を要求とする (ユーザー 2026-05-28 指摘「書いてあるのに余計なことをしている = デグレ要素、デグレ判定で出せるようにしないといけない」)
