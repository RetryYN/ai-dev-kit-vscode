...
contains_action_plans:
  ...
  - docs/plans/add-feature/add-feature-2026-06-16-c3a-fr-uses-forward-full-required.md  # §4.1 ③a自動化=fr_uses forward(uses先実在) を full-scan required 化（narrow per-detector flip, reverse は DF-P2-FRUSES-PROMOTE へ分離）
agent_slots:
  - role: se
...
| 2026-06-16 | **§4.1 自動化 ①②③a を 3 連続 LANDED**（個別 add-feature Action、各 gate-driven push 全8 gate green）: ①C-1 W1 narrow fail-close（`4eb9244`、standalone check_requirement_drift/check_g7_subcheck --gate）／②**C-2 ruff/shellcheck advisory CI job**（`5367c2c`、continue-on-error・Required 非対象、`forbidden_now` install/execute external tools を advisory-only 限定解禁＝ユーザー AskUserQuestion 承認）／③a **C-3a fr_uses forward(uses先実在) full-scan required**（`09e2b19`、narrow per-detector flip、blocking-only clean、reverse は DF-P2-FRUSES-PROMOTE へ分離、`forbidden_now` broad flip は不変＝narrow≠broad）。各々 L7 境界契約 evolution（add-feature count 19→20→21 同期、current_scope_authorized 追加、forbidden_now 5 項目不変）。**2026-06-16 ユーザー判断でここで区切る** = 残 §4.1（下記 step 3 残 detector / fr_uses reverse / step 4 右腕 / HELIX DB / branch-protection）は「automation 完了」と表現せず pending gate evidence として残す（TL 諮問 passed、退化防止＝新 roadmap 化しない）。 | PM (Opus) + Codex se + tl-advisor |
...
- **DF-DATEROT-BASENOW**（2026-06-18 発見、deferred）: `cli/lib/tests/test_harness_monitor_unit.py` の `BASE_NOW=2026-05-17` 固定挿入と本体 `datetime('now','-N days')` 実時刻窓の混在で date-rot（`test_feedback_loop_snapshot` が 06-16 を越えて fail）。C-3b push 時に**当該1テストのみ実時刻窓内挿入へ最小修正**（ユーザー裁定 2026-06-18、別 Incident commit）。**systemic 残**: `BASE_NOW` 40 箇所の latent rot 予備軍。恒久対策（fixture 時刻基準統一 or 本体 reference-now 注入 + **date-rot 検出テスト先置き**）は別 PLAN/Refactor として後続（TL 推奨: 今 systemic 改修は scope 越え）。本番コードは不変。
- **DF-P2-EXISTING-CYCLES**（2026-06-19、deferred、C-3e で baseline accepted floor 化）: `cli/lib` の既存 import 循環 5 件（例: `http_api/server.py`↔route 群、`agent_policy_guard.py`↔`team_runner.py` 等）。C-3e（dependency_cycle baseline-required）で `import-cycle-baseline.json` を **accepted floor** として可視化したまま残し、baseline 超の新循環のみ fail-close。**循環の実解消は本 ticket で別 Refactor PLAN（kind=refactor、振る舞い不変の保護網 + 回帰テスト + 段階分割、forward_return=L7/L8）として後続**。owner=PM、expiry=baseline TTL（切れ前に Refactor 起票 or 再 accept）。floor 化≠解消（設計劣化は残存、TTL で追跡）。
- **DF-P2-PLANDEP-WARNINGS**（2026-06-19、deferred、C-3d で baseline accepted floor 化）: `plan_dependency_gate` の accepted dependency warning 群（reciprocal / 型 / 軽微）。C-3d（plan_dependency baseline-required）で `plan-dependency-baseline.json` を accepted floor とし、baseline 超の新債のみ fail-close。**warning の実解消（PLAN 編集）は本 ticket で後続**。owner=PM、expiry=baseline TTL。`kind="other"` 分類の精緻化（full-scan block 対象の説明責任、TL P2）も本 ticket に含める。
- **DF-P2-DEFERRED-COUNT-DERIVE**（2026-06-19、deferred、C-4a G8 closure で判明）: 右腕 execution gate の `deferred_count` / `deferred_gates=[G8,G9,G12,G14]` が ~12 site（active ledger・current test-design contract・tests）に **hand-pin** され cross-consistency test で相互強制 = **SSoT 違反**。G8 closure で 1 gate 閉じるだけで全 site の手 sync が必要になった（C-4a で bounded 手 sync 実施）。**恒久対策 = `deferred_count` / `deferred_gates` を live VG（`check_vg_overview --strict-full-flow`）から derive し、ledger/doc は derive 値を参照する方針を別 PLAN（kind=refactor、forward_return=L7）で起票**（TL 推奨: 今 refactor 先行は blast radius 増 = G9/G12/G14 closure 前後で起票検討）。owner=PM。これを解消すれば G9/G12/G14 closure の手 sync コストが消える。
...
**自動化（先）** — 各々 `forbidden_now` 解禁が必要なものは entry にユーザー明示承認。**進捗（2026-06-16 時点）**: 1✅ / 2✅ / 3 部分（fr_uses forward のみ ✅、残 detector pending）/ 4 pending。
1. ✅ **LANDED**（`4eb9244` C-1）: **W1 狭い fail-close 昇格**（L6↔L7）。既に green な detector surface（requirement_drift / vg_overview / g7_subcheck）のみ fail-close 化。**broad flip はしない**（`forbidden_now: broad advisory→fail-close flip of W1 detectors` 抵触＝明示承認 entry）。
2. ✅ **LANDED**（`5367c2c` C-2）: **ruff/shellcheck advisory**（L6↔L7）。`continue-on-error` advisory のみ、required 化しない（`forbidden_now: install·execute external tools` を advisory-only 限定解禁＝ユーザー承認済）。
3. **W17/W18 ratchet full-required**（W17=L6↔L7 / W18=L4↔L9 + L6↔L7）: baseline 債を **detector 単位**で解消し `changed-files ratchet → detector 単位 full-required`。一括 full は CI red（P1）。**進捗**: ✅ **C-3a `09e2b19`** = fr_uses forward(uses先実在) を full-scan required（narrow per-detector flip、blocking-only clean）。**pending gate evidence（残）**: dependency_cycle（import循環5＝Refactor 前提）/ plan_dependency（warning49＝PLAN 編集前提）/ coding_rule_lint（core=bash-n/py_compile と optional=ruff/shellcheck の分離が前提、ruff/shellcheck を required path に混入させない＝C-2 advisory 境界遵守）/ **fr_uses reverse（逆参照）= DF-P2-FRUSES-PROMOTE**（forward edges からの derived index 算出＋`missing_reverse_reference` 契約 evolution、手書き双方向は片方向正本と衝突＝不可）。
4. **W2 右腕 execution gate**（G8=L5↔L8 / G9=L4↔L9 / G12=L3↔L12 / G14=L1↔L14）: **左腕設計/検証設計の再凍結を先行**してから L7 配線・CI required。L7/CI から始めると片肺化（P1）。最長工程。**pending gate evidence（残）**。
