...
> **本 PLAN の対象**: L1 [業務要求 doc](../../v2/L1-requirements/helix-workflows-business-requirements.md) BR-01〜BR-08 を **業務フロー (確定版) + 業務ルール + 対象業務範囲** で詳細化し、L12 受入テスト設計とペア凍結する。HELIX-workflows V2 dogfooding における L3 の「業務要件」を確定する。L1 では要望レベル、L3 で実装可能な確定版に昇格。

## §1 工程表 (作業手順 + 進捗)

| Step | 作業内容 | 進捗 |
|---|---|---|
| 1 | 参考調査 (L1 業務要求 BR-01〜BR-08 詳細精読 + L0 概念 + L1 4 PLAN G1 結果 + L3 template) | ☑ completed (2026-05-26、L1 commit aa86a22 受け継ぎ) |
...
| 4 | L12 受入テスト設計 pair artifact 起草 (各 BR-* → AC-BR-* mapping、`docs/v2/L12-test-design/helix-workflows-acceptance-test-design.md`) | ☑ completed (2026-05-26、§1 AC-BR-01〜08 detail 化、§2/§3 は Phase E.B 完了後に detail 化 carry) |
| 5 | TL レビュー (helix codex --role tl-advisor、adversarial check 1 回必須、G3 evidence) | □ pending (Phase E.B 並走 2 PLAN 完了後にまとめて adversarial check 投入予定) |
...
|---|---|---|
| BR-01 dogfooding 稼働 | 新規 PLAN 起票 → V-model 整合 check → fail-close 自動差戻し業務フロー | AC-01: NSM 計測 SQL + Guardrail GR-1 fail-close 動作確認 |
| BR-02 4 artifact retrofit | 既存 PLAN scan → 4 artifact 不在検出 → retrofit 自動 stub 生成業務フロー | AC-02: helix doctor warn 数推移 + retrofit 完遂率 |
| BR-03 drift 解消 | 週次 detector → 新規 drift → Reverse normalization mode 自動切替業務フロー | AC-03: detector 検出 + normalization 完了時間 |
| BR-04 9 mode → Forward 回帰 | mode closure event → helix.db.mode_transition 登録 → 適切 L 工程接続業務フロー | AC-04: mode_transition event 登録率 + Forward 接続成功率 |
| BR-05 ペア凍結監査 | parent_design / pairs_test_design 不在検出 → frontmatter 自動 lint 業務フロー | AC-05: ペア凍結 coverage 5 pair `balance_ratio` 計測 |
| BR-06 影響範囲分析 | 機能改修 trigger → 4 artifact 双方向 trace query → 影響範囲 視覚化業務フロー | AC-06: query 応答時間 + trace 網羅率 |
| BR-07 AI agent 配線 | L 工程 entry → vmodel-semantics.yaml 読込 → mandatory_skills/commands 自動注入業務フロー | AC-07: 注入セット利用率 + AI 判断削減率 |
| BR-08 採用 project 展開 | HELIX-workflows portable package → 採用 project 取込 → 各 project の dogfooding 起動業務フロー | AC-08: 採用 project 数 + 各 project の OT-01 相当稼働率 |
...
- [ ] §2.2 L1 → L3 詳細化 mapping 8 件 (BR-01〜BR-08) すべて確定
...
- [ ] G3 要件凍結ゲート acceptance criteria 満足 (PM + PO 判定、業務要件 + 受入テスト設計 ペア凍結)
