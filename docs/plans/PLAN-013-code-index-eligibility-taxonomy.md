...
build 時は `path`/`line_no` の再取得を前提に再分類を再現可能とする。
...
- 再判定は `path + line_no` から当該行を再取得して再構成可能。
...
- `--uncovered` の **判定 (covered/uncovered)** は、各 symbol について `.helix/cache/code-catalog.jsonl` に対応する entry (path + line_no で同定) があるかで決まる。
...
- `--uncovered` 出力 schema:
  - **uncovered entry** (JSONL に entry なし): `path` / `line_no` / `symbol` / `kind` / `bucket` / `seed_candidate` (default 算出値) / `seed_promotable` (default false 固定)
  - **covered entry** (JSONL から取得): `id` / `domain` / `summary` / `since` / `related` / `path` / `line_no` / `source_hash` / `updated_at` / `bucket` / `metadata.seed_candidate` / `metadata.seed_promotable`
...
`code-catalog` が保有する `symbol` ではなく `path+line_no` の再スキャン再現性が migration 品質を支える。

- 対策: v14→v15 migration を fixture 化
- `path + line_no` で再取得後に bucket 判定を再現できることを担保
- 再分類後も id 一貫性を担保
...
v1.4 で approve 到達。残 P2/P3 (4件) は G3 Schema/API Freeze までに対応する課題として next_steps へ持ち越し:

- P2 F1: `line_no` の canonical semantics (marker_line vs symbol_line) と covered/uncovered matching key を G3 で凍結
- P2 F2: CLI JSON 外部契約を flat item に統一するか nested metadata にするかを G3 で明記
- P2 F3: PLAN-012 の対象外 path (test_*.py / tests/ / fixture/ / generated/ / vendor/) を PLAN-013 の pre-filter として継承するかを G3 で明文化
- P3 F4: 解消済 (本 v1.4 で §3.3 subsection 番号を 3.3.1 → 3.3.2 → 3.3.3 → 3.3.4 の昇順に修正)
