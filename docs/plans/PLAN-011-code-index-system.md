# PLAN-011: コード index 登録システム (v1.1)
...
- 多言語対応の網羅 (本 PLAN は Python / bash / markdown / bats のみ。TypeScript / Go は次 PLAN)
...
走査対象: `git ls-files` で取得した tracked files のうち `*.py`, `*.sh`, `*.md`, `*.bats` (本 PLAN scope)。
直近 1 行下の Python `def` / `class` / bash function、または markdown の見出しに紐付く。
