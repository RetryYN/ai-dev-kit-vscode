...
|---|---|---|
| **C1 schema 単一 registry SSoT** | DDL を単一 registry から生成（harness base は **SCHEMA_VERSION=18 / 56 table**、V3 設計は **62 table** = base +`test_result_events` append / +`functional_registry` projection、personal +`template_catalog` / `doc_coverage` / `prompt_interpretations` / `learning_candidates` projection。実装時は V3 registry の `SCHEMA_VERSION` を bump）。識別子 fail-close（import 時 `assertSqlIdentifier`）。**table を projection ⊥ append_event ⊥ config に分類**（TL C-1、分類 SSoT = [C1 §5](engine/schema-registry.md)） | plan_lint/validator 二重管理 drift・schema 分散 → 根絶 |
...
|---|---|---|
| **D1 DB/projection・D3 schema** | harness 56-table clean registry → V3 62 / projection⊥append_event⊥config | **Python で忠実に再構築 + HELIX personal extension を projection table として上乗せ**（C1/C2、TL C-1/C-2 凍結） |
...
docs/v3/
  ...
  engine/                ← keystone 設計（clean harness 由来・Python build）
    schema-registry.md       C1: 62-table（harness 56 + V3 base 2 + personal 4）単一 registry / projection⊥append_event⊥config 分類 SSoT / enum / 識別子 fail-close
  ...
  L0-L14/                ← V-model 設計（harness 由来・新規再構築）
    ...
    L5-detailed-design.md    module 分解 / physical-data(62 table)/ if-detail(D-CONTRACT)/ DbC freeze
