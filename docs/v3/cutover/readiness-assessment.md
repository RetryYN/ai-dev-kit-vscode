...
### 2. cutover-gate machine verdict

- **非破壊 config（retire=()）+ rollback 機構 wire → gate ok=True**（pin/dangling/rollback_preflight/rebuild_dry_run 全緑）= **機械的 cutover-readiness は達成**。
- **ただし gate は parity を検査しない（gap）**: 実退役 config（retire=V2 の test/detector）でも gate は機械的に緑になりうるが、V3 が退役対象を cover しているかは見ていない。**緑 gate ≠ 破壊安全**。
...
2. **gate に parity floor を追加**（現 gap 是正）: 退役 inventory 非空時は V3 coverage 検証を必須化（緑 gate を破壊安全と等価にする）。
