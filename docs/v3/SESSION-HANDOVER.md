...
1. **DB = harness 56-table → V3 base 58 → personal edition 設計 62**（旧「core 14 + Phase2」を訂正）。**V3 base 58** = harness 56 + 追加 2（`test_result_events`(append) + `functional_registry`(projection)）。**personal edition 設計 62** = base 58 + personal 4 projection（`template_catalog` / `doc_coverage` / `prompt_interpretations` / `learning_candidates`、契約 = [personal-edition-schema-contract](engine/personal-edition-schema-contract.md)、`implementation_status=L7-carry`）。分類 SSoT = C1 §5 = **projection 53 / append_event 3 / config 6 = 62**（TL C-1）。append_event 3 = `test_result_events` / `guardrail_decisions` / `hook_events`（capture §103 = red-first/review-guard/hook-bypass のみ）。`baseline_registry` は table でなく C5 frozenset、`state_events` は table でなく `screen_trace`。**実装(`cli/lib/v3`)は現状 base 58**（personal 4 は L7 で registry append + `SCHEMA_VERSION` bump、§下「実装状況」の 58 参照はこの impl 値で正）。
