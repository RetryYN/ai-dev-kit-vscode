...
- **PLAN**: 旧形式 PLAN(PLAN-NNN) 224 件 (`is_reference: true` 参考扱い) + 新形式 PLAN(L<NN>-○○○plan) 100 件
...
- 既存の旧形式 PLAN(PLAN-NNN) 224 件の新命名 retrofit: `is_reference: true` 維持、製本したい場合は新命名規則(L<NN>-○○○plan)で新規書き直し
...
flowchart TB
  ...
  L11 -->|G11: RC 判定| L12[L12 受入テスト]
  L12 -->|G12: 受入判定| L13[L13 運用検証 / 運用テスト]
  L13 -->|G13: 安定性| L14[L14 運用学習 / 運用改善]
