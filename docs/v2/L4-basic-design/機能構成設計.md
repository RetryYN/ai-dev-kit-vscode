...
**昇格タイムライン (L4 carry → L5 → approved L7)**: FR-FNREG-01 (`cli/config/functional-registry.yaml` + `helix function registry` + `helix doctor check_fr_sot_alignment`) と FR-GLOSSARY-01 (`cli/config/glossary.yaml` + `helix glossary` + `helix doctor check_glossary_coverage`) の 3 点充足は **L4 carry** とし、L5 詳細設計で SSoT schema を確定する。code への昇格は L7 実装だが、現在スコープでは実施せず、承認済み add-feature / PLAN を入口にする。昇格判定は対応 `helix doctor` check の実装をもって機械確認する。各 FR の実装状況 (installed / partial / L4-carry / not-implemented) の詳細は [L0 §12 Glossary](../L0-helix-workflows/concept.md) と [L3 functional-registry](../L3-requirements/helix-workflows-functional-registry.md) を正本とする (本書は機能群構成の俯瞰であり、per-FR implementation_status の SSoT は持たない)。
