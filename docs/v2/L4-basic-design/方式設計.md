...
L4 では各 NFR 群を §5 ビルディングブロック単位の方針へ落とす (コンポーネント粒度)。閾値・計測窓・detector 実装詳細は L5 / L4↔L9 総合テスト設計へ送る。L3 [非機能要件](../L3-requirements/helix-workflows-nfr-detail.md) の 6 群 (AV / PF / OP / MG / SC / SE) すべてを本表で受ける。

| NFR 群 | 担当ビルディングブロック (§5) | コンポーネント粒度の対応方針 | 参照要件 | implementation_status |
|---|---|---|---|---|
| 可用性 (AV) | Audit and Persistence / 統合層 | `helix.db` は単一接続のローカル SQLite とし WAL + トランザクション境界で整合性を担保 (NFR-AV-02: 整合性 100%)。hook / HTTP route の局所障害は統合層で fail-close し、Bash router・Python application 層へ波及させない | NFR-AV-01, NFR-AV-02, NFR-AV-03 | partial (WAL / 整合検査の確定は L5) |
| 性能 (PF) | Workflow Control / Audit and Persistence | 影響範囲 query は `helix.db` の index 済 trace テーブル経由で 5 秒以内 (NFR-PF-01)。doctor は check 単位で分割し局所実行可能にする (NFR-PF-02)。重い集計は view へ寄せる (NFR-PF-03/04) | NFR-PF-01, NFR-PF-02, NFR-PF-03, NFR-PF-04 | partial (index / view 設計は L5) |
| 運用保守 (OP) | 全ブロック横断 | 文書正本 (`docs/`, `HELIX-workflows/`)・CLI (`cli/`)・DB schema の責務を分離し双方向 trace を維持 (NFR-OP-04)。warn は段階管理し Phase exit 条件と連動 (NFR-OP-03)。doctor 集約で健全性を観測 (NFR-OP-01/02/05) | NFR-OP-01, NFR-OP-02, NFR-OP-03, NFR-OP-04, NFR-OP-05 | partial |
| 移行・互換進化 (MG) | Audit and Persistence (schema) | **Strangler Fig 方式の段階移行 (FR-MIGR-01)**: 既存 schema を破壊せず `schema_version` テーブルで版管理し、migration は冪等 (再適用安全) を L4 方針として固定 (NFR-MG-02)。旧→新は並存期間を設け drift を advisory 検出 (NFR-MG-01/03)。migration 単位・rollback 手順の確定値は L5 物理データ設計 ([データ設計.md](./データ設計.md) §6) へ送る | NFR-MG-01, NFR-MG-02, NFR-MG-03 | partial (`schema_version` 実在、冪等 framework は L5) |
| セキュリティ (SC) | Runtime Governance | raw CLI 抑止・hook guard・commit/push guard を harness 側で fail-close 強制 (NFR-SC-03/04)。role / model / path は `*_guard.py` で機械強制 (NFR-SC-01/02)。secret / PII は commit guard で遮断 (NFR-SC-05) | NFR-SC-01, NFR-SC-02, NFR-SC-03, NFR-SC-04, NFR-SC-05 | implemented (guard 群実在) |
| 互換性 (SE) | Bash router / 統合層 | Linux / macOS 前提、配置非依存 mount (`~/.helix/core`) で path 非依存。Codex / Claude 両導線を wrapper で吸収 | NFR-SE-01, NFR-SE-02, NFR-SE-03 | implemented |

> NFR 閾値の合否判定 (可用性整合率 100% / 影響範囲 query ≤ 5 秒 / warn 件数しきい値 等) は L4↔L9 pair の [総合テスト設計](../L9-test-design/L4-basic-design-総合テスト設計.md) で検証設計する (pair freeze)。
