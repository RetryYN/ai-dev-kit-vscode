...
### 5.3 NFR 保証方針 (L4: 整合性・移行)

データ層が満たすべき NFR の L4 方針を固定する (閾値・DDL・migration script は §6 のとおり L5 物理データ設計へ送る)。

| NFR | データ層での保証方針 (L4) | 接続点 |
|---|---|---|
| NFR-AV-02 (helix.db 整合性 100%) | 単一接続のローカル SQLite を WAL モードで運用し、書込みはトランザクション境界で原子化する。PLAN / run / audit の関連は参照規約で保ち、破損・schema mismatch は起動時 / `helix doctor` で検知する | [方式設計.md](./方式設計.md) §7 可用性、[外部IF設計.md](./外部IF設計.md) §6 (SQLite mismatch 検知) |
| NFR-MG-02 (schema migration 冪等) | `schema_version` テーブルで適用済 version を管理し、migration は再適用安全 (冪等) を方針とする。既存 schema を破壊せず追加優先 (**Strangler Fig = FR-MIGR-01**) で段階移行する | [方式設計.md](./方式設計.md) §7 移行 (MG)、FR-MIGR-01 |
| NFR-MG-01 / 03 (移行追跡・drift) | 旧→新の並存期間を設け、migration 進行を追跡し drift を advisory 検出する | FR-MIGR-01, FR-DRIFT-01 |

migration 単位・index・列定義・`schema_version` 更新規約・rollback 手順の確定値は §6 のとおり L5 物理データ設計が正本。
