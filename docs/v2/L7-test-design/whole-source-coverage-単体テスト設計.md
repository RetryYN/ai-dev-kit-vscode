...
| UT-WSC-02 | FN-WSC-02 | posttooluse-design-doc-web-search-revert.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 部分 (Python pytest 済 / hook E2E 未, carry) |
| UT-WSC-03 | FN-WSC-03 | posttooluse-helix-job-enqueue.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
| UT-WSC-04 | FN-WSC-04 | posttooluse-plan-auto-register.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 部分 (Python pytest 済 / hook E2E 未, carry) |
| UT-WSC-05 | FN-WSC-05 | posttooluse-skill-catalog-rebuild.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
| UT-WSC-06 | FN-WSC-06 | precompact-state-snapshot.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
...
| UT-WSC-10 | FN-WSC-10 | pretooluse-codex-slot-check.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
| UT-WSC-11 | FN-WSC-11 | pretooluse-design-doc-web-search-guard.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 実装済 |
| UT-WSC-12 | FN-WSC-12 | pretooluse-opus-repo-block.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 部分 (Python pytest 済 / hook E2E 未, carry) |
| UT-WSC-13 | FN-WSC-13 | sessionstart-harness-summary.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
| UT-WSC-14 | FN-WSC-14 | sessionstart-history-injection.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 実装済 |
| UT-WSC-15 | FN-WSC-15 | stop-recovery-update.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 部分 (Python pytest 済 / hook E2E 未, carry) |
| UT-WSC-16 | FN-WSC-16 | stop.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 実装済 |
| UT-WSC-17 | FN-WSC-17 | userpromptsubmit-context-bundle.sh | hook の fail-close/fail-open 方向 + ensures(verdict/副作用) を反証 | 未整備 (carry WSC-TEST-IMPL) |
...
| UT-WSC-213 | FN-WSC-213 | uuid7_generator.py | 主関数の invariant(エラー時/fail-close) + ensures(戻り値/出力) を反証 | 未整備 (carry WSC-TEST-IMPL) |
...
| UT-WSC-218 | FN-WSC-218 | zizmor_ignore_lint.py | 主関数の invariant(エラー時/fail-close) + ensures(戻り値/出力) を反証 | 未整備 (carry WSC-TEST-IMPL) |

## carry: WSC-TEST-IMPL（テスト実装未整備 = 12 件、設計 freeze と分離）

> **設計の抜け漏れではない**。FN-WSC/UT-WSC 54 の設計は完備（trace_symmetry L6↔L7 green）。以下はテスト**実装**の carry（L7 sprint で実装）。machine-clean な design freeze は成立済み。

| 区分 | FN-WSC | 件数 |
|---|---|---|
| 専用テスト無し（hook 6 + lib 2） | 03, 05, 06, 10, 13, 17, 213(uuid7_generator), 218(zizmor_ignore_lint) | 8 |
| Python pytest 済 / hook script E2E(bats) 未 | 02, 04, 12, 15 | 4 |
| **合計 weak** | — | **12** |
...
- weak 12 件は `WSC-TEST-IMPL` carry（L1 verification-strategy §13 と件数一致）。
