...
| BR-09 既存資産整理 | OT-09 inventory drift 率 (L14 起票時定義) | 毎月末 |
| BR-10 段階移行 | OT-10 V1 移行率 (L14 起票時定義) | 毎月末 |
| BR-11 doc レビュー | OT-11 doc-reviewer 召喚率 (L14 起票時定義) | 毎週 |
| BR-12 デグレ禁止 | OT-12 デグレ件数 (L14 起票時定義) | 毎週 |
...
- **BR-09〜BR-12 の L14 OT-09〜12 対応**: §7 表に pair 宣言済 (2026-05-28 L0→L1 完遂検証で補充)。OT-09〜12 の実体定義は L14 運用テスト設計 doc 起票時に追加する carry。L1↔L14 pair freeze の balance_ratio 実測は L14 工程で完成 (今は L0→L1 フェーズのため L14 doc 本体には未着手)。
