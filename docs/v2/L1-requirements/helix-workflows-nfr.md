...
- **L3 接続規約 (2026-05-26 tl-advisor G1 P1 #2/#3 反映、4 L1 doc 共通)**: `L3-helix-workflows-requirementsplan` は L1 4 PLAN 全件 (業務 / 機能 / 技術 / 非機能) を `dependencies.requires` に列挙し、`docs/v2/L12-test-design/helix-workflows-acceptance-test-design.md` を pair artifact として同時起票して L3 非機能要件 (IPA グレード値) と L12 受入テスト設計の NFR 系 AC-* を pair freeze する (詳細は業務要求 doc §7 参照、L4↔L9 + L13 + L14 は更に下流の多層検証)
