...
freeze_layer_pair: "L5↔L8"
freeze_date: "2026-05-29"
balance_ratio_note: "IT-IP 73 / IT-MOD 66 / IT-DB 60 / IT-IF 168 件で L5 設計 4 doc を網羅 (pair ratio ≥1.0)"
---
...
|---|---|---|---|
| IT-IP | F1〜F10 の内部処理フロー (中核 F1-F5 + governance F6-F10) | PLAN frontmatter, mode routing, governance hook, 経路移譲 | 失敗時は phase ごとの KPI 低下と statusLine 欠落、または hook 逸脱 |
...
L5 R1 修正で本文定義 36 件に統一 (R1 audit C1 反映、commit d5b611c)。CLI 36 コマンドの exit code を mode / path 別に再定義し、仕様と差分がないことを検証する。IT-IF-CLI ケースは exit code 体系を共有するコマンドをグルーピングするため 34 件 (IT-IF-CLI-001〜034) で 36 コマンドを網羅する (1:1 ではない)。
