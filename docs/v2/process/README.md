...
**L7 は PLAN の上位概念**。L7 が全体の進捗を管理する。**PLAN は全工程 (L0-L14) で機能 (ドキュメント) 単位に起票**し、ある工程から次の工程へ進むために「どのドキュメントを・どの順番で・どんな手順を踏んで作るか」を **工程表として定義**する。PLAN は **工程表 (進捗) + 実装計画** の両方を内蔵し、作業が中断してもどこまで進んだかを再開可能にする。
