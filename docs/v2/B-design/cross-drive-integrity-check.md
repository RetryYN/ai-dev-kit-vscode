...
|---|---|---|---|---|---|---|
| CI-001 | artifact naming | fe | fullstack | FE architecture は `state-events.md`、fullstack architecture は `state_events`、fullstack requirement は `state_event_scope`。artifact ID とファイル名と scope 名が同列に混在。 | artifact ID は snake_case に統一し、実ファイル名は `examples.path` へ退避する。requirement は `state_event_scope`、architecture は `state_events` を正本化。 | **resolved (commit pending, fe-draft state-events.md → state_events)** |
