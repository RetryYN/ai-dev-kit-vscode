...
| Phase | 起点モジュール | 主処理 | 出力 |
|---|---|---|---|
| Entry | `cli/helix` | サブコマンド名を解決し `exec` で専用 entrypoint に委譲する | コマンド別 shell/Python 実行 |
| Guard | `context_guard.py`, `llm_guard.py` | 必須 context、raw CLI、hook 整合を検査する | 許可/拒否 verdict |
| Orchestration | `route_engine.py`, `plan_validator.py`, `workspace_manager.py`, `handover.py` | mode ルーティング、PLAN 検証、workspace 操作、handover 継続 | 実行方針、状態更新、再開情報 |
| Persistence | `helix_db.py`, `migrations/*.py` | run、audit、session、workspace の状態を SQLite に保存する | `helix.db` row 群 |
| Automation | `http_api/server.py`, `routes/push_pr.py`, `routes/hooks.py`, `routes/audit.py`, `routes/telemetry.py` | 外部 trigger を内部 run/audit 更新へ変換する | `automation_runs` / `audit_log` / `session_telemetry` |
