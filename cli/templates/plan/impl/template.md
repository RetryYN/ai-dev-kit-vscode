...
kind: impl
layer: L7              # 現行 15 工程: L7 実装スプリント
drive: be
status: draft
created: 2026-MM-DD
owner: PM
process_layer: L7   # ★必須: 本 PLAN は L7 実装スプリント工程の subordinate (docs/v2/process/L07-implementation-sprint.md 参照)
parent_design: docs/v2/L6-functional-design/<feature>/<function>.md  # ★必須: L6 機能設計 doc への path (L0-L6 設計工程で確定済の親 doc)
...
- L7 実装スプリントでは Entry → Exit の検証順序をテンプレート上で固定し、再現可能性を上げる。
