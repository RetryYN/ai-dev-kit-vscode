...
- **駆動モデル別 closure DB（park、2026-06-02 順序判断）**: `mode_transition` 等の closure event schema は本モデルに**含めない**。駆動モデル別 DB は Forward 従属の補助層（Core「補助 state → closure event → V-model DB merge」）であり、正当化されたとき（具体的な駆動 workflow が runtime closure 記録を要求し、closure モデルが妥当と確認できたとき）に L4 closure/data 設計で**意図的に**作る。escalation blocker ではなく順序 park。SSoT lean = `mode_transition`、既存 `reverse_local_loops`/`scrum_local_loops` パターン参照（[poc §4.5](../../docs/plans/discovery/poc-2026-06-01-recovery-closureplan.md)）。
