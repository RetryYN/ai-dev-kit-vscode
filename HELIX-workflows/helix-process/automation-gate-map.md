...
- `fr_uses_checks`（FR-LIB-159 / FN-WSC-228）: functional-registry `uses` field（片方向正本）の uses 先実在を fail-close。**forward(uses先実在)は C-3a（下記）で full-scan required へ昇格済**。逆参照欠落は derived warning（reverse required 化は別 Action DF-P2-FRUSES-PROMOTE、§5）。
- 段階: `coding_rule_lint` / `dependency_cycle_checks` / `plan_dependency_gate` は **changed-files ratchet（新規違反のみ block）**。`fr_uses_checks` forward は **C-3a で full-required（全件 fail-close）へ昇格済**。他の full required 化 / CI required 化 / 逆参照必須化は deferred（weakness-map DF-P2-*）。
- **fr_uses forward の full-required 昇格（C-3a、§4.1 ③a、2026-06-16、ユーザー承認の narrow per-detector flip）**: `vg_overview.required_clean.fr_uses_checks` の source を changed-files ratchet（`collect_fr_uses_gate_summary`）から **full-scan full-required（`collect_fr_uses_full_required_summary`）** へ切替。**`blocking_finding_count == 0` を clean** とする（forward=uses先実在の全件 fail-close。reverse の missing_reverse_reference warning は clean 判定に算入しない）。changed-files availability に依存しない。**reverse(逆参照)の required 化は含まない**（片方向正本維持、derived 算出は別 Action DF-P2-FRUSES-PROMOTE）。他 3 detector の required_clean source は不変。これは forbidden_now #5「broad advisory→fail-close flip of W1 detectors」に**抵触しない**（narrow per-detector flip、forward-only）。境界契約 `current_scope_authorized` に C-3a 追加済（forbidden_now 5 項目は不変）。正本 = [add-feature-2026-06-16-c3a-fr-uses-forward-full-required](../../docs/plans/add-feature/add-feature-2026-06-16-c3a-fr-uses-forward-full-required.md)。
