...
- 判定 (`required_clean` 全 clean): `registry_design_coverage`（whole-source⊆design）+ `source_scan_vs_registry` + `registry_trace_complete` + `requirement_drift` + **`fn_ut_pair_coverage`**（FN↔UT 1:1 網羅）+ **`design_id_existence`**（FN-* の L6 doc 実在）+ **`ddd_bc_coverage`**（DDD bc 2 check）+ **`coding_rule_lint`** / **`dependency_cycle_checks`** / **`plan_dependency_gate`** / **`fr_uses_checks`** + `trace_symmetry all applicable pairs clean/semantically-accepted` + `未承認 P0/P1 deferred finding = 0`（PM 承認なき限り）。required path の mode は、`fr_uses_checks` / `coding_rule_lint` が **full-required**、`dependency_cycle_checks` / `plan_dependency_gate` が **baseline-required**（既存 baseline=floor、baseline 超の新債のみ fail-close）。
...
### 3.4 コードルール / 依存関係 detector（pre-L7 ゲート硬化 Phase2、2026-06-14）
...
- `dependency_cycle_checks`（FR-LIB-157 / FN-WSC-226）: cli/lib import 循環。**C-3e で `baseline-required`** に昇格済み。`import-cycle-baseline.json` の既存 5 循環を accepted floor として保持し、**baseline 超の新循環のみ fail-close**。これは **full-required ではない**（既存債は可視化継続、解消は Refactor で別途）。
- `plan_dependency_gate`（FR-LIB-158 / FN-WSC-227）: `plan_validator` の dependency 検証（型/self-edge/reciprocal/cycle）の wrapper。**C-3d で `baseline-required`** に昇格済み。`plan-dependency-baseline.json` の accepted floor を超える **新規 dependency finding 全件** を fail-close とし、`changed-files` には依存しない。実在 path 参照の false-positive は baseline 吸収せず detector 側で修正する。これは **full-required ではない**。
...
- 段階: `fr_uses_checks` は **C-3a/C-3b で forward + reverse(derived drift only) の full-required（全件 fail-close）**、`coding_rule_lint` は **C-3c で core(bash_n/py_compile) の full-required**、`dependency_cycle_checks` / `plan_dependency_gate` は **C-3d/e で baseline-required（既存 baseline=floor, baseline 超の新債のみ fail-close）** へ昇格済。`baseline-required` は `full-required` と別概念であり、既存債の burn-down は Refactor / PLAN 編集の別 ticket で追う。optional ruff/shellcheck は C-2 advisory 据置。
...
- **plan_dependency_gate / dependency_cycle_checks の baseline-required 昇格（C-3d/e、§4.1 ③d/e、2026-06-19、ユーザー承認の narrow per-detector flip）**: 両 detector は `changed-files` ratchet から **full-scan baseline-required** へ切替。`clean = blocking_finding_count == 0` で、blocking は **baseline 超の新債のみ**。既存 baseline は accepted floor として可視化を維持し、ここでは **full-required（全件 fail-close）へは昇格しない**。`plan_dependency_gate` は実在 path 参照の false-positive を detector 側で修正してから baseline を評価する。境界契約 `current_scope_authorized` に C-3d/e 追加済。正本 = [add-feature-2026-06-19-plandep-depcycle-baseline-required](../../docs/plans/add-feature/add-feature-2026-06-19-plandep-depcycle-baseline-required.md)。
