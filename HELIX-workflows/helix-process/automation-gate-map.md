...
- 段階: `coding_rule_lint` / `dependency_cycle_checks` / `plan_dependency_gate` は **changed-files ratchet（新規違反のみ block）**。`fr_uses_checks` は **C-3a/C-3b で forward + reverse(derived drift only) の full-required（全件 fail-close）** へ昇格済。他の full required 化 / CI required 化は deferred（weakness-map DF-P2-*）。
