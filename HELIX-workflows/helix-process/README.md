...
中核の Forward（Vモデル L0–L14）、入口に応じた workflow、FE/UX 工程の専門2、そして管理・自動化ドキュメント群で構成される。すべての workflow は最終的に Forward のドキュメント体系へ収束・昇華する。
...
| ドキュメント構成と参照関係 | document-topology.md | 文書を作る・修正する時の配置、参照方向、常時注入 / 詳細注入の判断 |
...
├── scrum / discovery / reverse / incident / add-feature-workflow.md   # 入口 workflow
├── refactor / retrofit / research / recovery-workflow.md              # 入口 workflow
...
├── document-topology.md                                               # 文書構成と参照関係
└── README.md                                                          # 本索引
```

## 設計思想

HELIX は TDD（テストファースト）を基本とする。コードを書く前に合格基準となるテストを先に書く順序を、全工程・全 workflow で厳守する（詳細は [HELIX-process-L0-L14.md](../HELIX-process-L0-L14.md)）。


多様な入口（新規 / 反復 / 探索 / 逆引き / 緊急 / 追加 / 構造改善 / 移行 / 調査 / 暴走対応）から始めても、最終的に Forward の単一ドキュメント体系へ収束する。逸脱は kind 付き PLAN の起票として記録され、自動登録で本線 DB に取り込まれ、検出（drift / 劣化 / 暴走 / 障害）が再び workflow を起動する。入口の柔軟さと Vモデルの厳格さ（双方向 trace・ゲート）が、単一 DB の上で両立する。
