...
- **承認の置換**: `helix push --gate --execute --plan-id <PLAN>` が **8 gate 全 PASS → authorized push**（別途の人間承認は不要）。これは HELIX harness 経由の push のみに適用する。
- **8 gate**: `G-tests` / `G-catalog` / `G-secret` / `G-ff` / `G-attr`(Co-Authored-By) / `G-nondestructive` / **`G-review`** / **`G-vg-overview`**。`G-review` は **`plan_scope` 別 lifecycle** で検査する（TL 判定 A, 2026-06-05）:
...
- **`G-tests` tier**: 既定は `auto`。`cli/lib/<mod>.py -> cli/lib/tests/test_<mod>*.py`、`cli/<script> -> cli/tests/*<script>*.bats` の保守 bucket selector を使い、局所変更では light にする。次の条件では `tier=full` に fail-close: `--full` / `--test-tier full`、`main` / `release/*` / `--allow-main`、changed-files source unavailable、selector 空だが code 変更あり、FULL_TRIGGER（shared core `push_gate.py` / `changed_files.py` / `vg_overview.py` / `helix_db.py` / `plan_validator.py`、`pyproject.toml` / `requirements*.txt` / `.github/workflows/*` / `cli/lib/tests/conftest.py` / `cli/helix-test` / bats helper or runner / `D-CONTRACT` / `github-operations.md` / `push.md` / `functional-registry.yaml` / `automation-gate-map.md` / contract mirror）および test delete / rename。`tier=full` は従来の full pytest + 全 bats を完全再現する escape hatch。
- **dogfood CI backstop**: `ci.yml` は `push` の `dogfood` と `feature/**` でも full pytest + 全 bats を回す。local push gate の `tier=auto` はここを backstop にしたうえで高速化する。
