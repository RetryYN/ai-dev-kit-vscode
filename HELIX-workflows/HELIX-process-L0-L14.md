...
**設計思想（カバー網）**: これらのモードは、AI エージェントが新規開発（Forward）以外の多様な状況に直面したとき、それを受け止めて Forward 正本へ収束させる**カバー網**として設計されている。既存資産の逆引き（Reverse）、追加機能（Add-feature）、本番障害（Incident）、要件未確定（Discovery）、構造改善（Refactor）、基盤改修（Retrofit）、技術調査（Research）、そして **AI 自身の暴走・独断専行・工程逸脱・認識ズレ（Recovery）**——新規開発の枠から外れた事象は、必ずいずれかのモードが引き受け、Forward の L0–L14 ドキュメント体系へ戻す。各モードのカバー範囲は下表「入口」列に対応する。

> **適用順序（重要）**: ある事象に直面したら、まず「**どのモードがこれをカバーするか**」を判定し、そのモードの入口から入る。例えば「既存システムへの機能追加」は Add-feature、「AI が暴走・工程逸脱した」は Recovery が引き受ける。**いきなり Forward 下流の設計（L4 基本設計 / L5 詳細設計 / L6 機能設計）や ADR に手をつけてはならない**。ユーザーが「こうあるべき」と求めたものは要求（L1）が起点であり、要求 → 要件（L3）→ 設計（L4→L5→L6）と V モデル順で降ろす。ADR は技術選定・アーキテクチャ転換のような滅多にない不可逆判断のみに使い、設計内容（schema / routing 表 / 関数仕様）や要求を ADR に詰めない（それぞれ設計書 doc / L1 要求に書く）。
