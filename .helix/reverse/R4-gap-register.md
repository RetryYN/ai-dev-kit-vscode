...
| ID | Gap 内容 | 種別 | 深刻度 | Forward 接続先 | 根拠 |
|----|---------|------|--------|--------------|------|
| GAP-001 | gate-checks.yaml パースに PyYAML が残存（helix-gate L428） | defect | P1 | L4 | R2 TD-001 |
| GAP-002 | helix-gate AI セクションの Bash 行パースが脆弱（インデント崩れ・特殊文字で誤動作） | defect | P1 | L4 | R2 TD-002 |
| GAP-003 | helix-task の sed による YAML 更新（同一パターン複数マッチで誤更新リスク） | defect | P1 | L4 | R2 TD-003 |
| GAP-004 | exit code 定義（EXIT_SUCCESS 等）が一部コマンドで未活用（helix-scrum, helix-task が exit 1 ハードコード） | defect | P2 | L4 | R2 TD-006, R1 common.exit_codes |
| GAP-005 | __pycache__ が git tracking に残存 | defect | P0 | L4 | R2 TD-009 |
| GAP-006 | SQLite FK 制約が未 enforce（PRAGMA foreign_keys = ON 未設定） | defect | P1 | L4 | R0 unknowns.spec_unclear, R0 unknowns.potential_risks |
| GAP-007 | gate_runs.task_run_id の FK が実 DB で未作成（定義と実態の乖離） | defect | P1 | L4 | R0 unknowns.spec_unclear |
| GAP-008 | Bash エラーハンドリング不統一（set -eo pipefail / set -euo pipefail 混在） | quality | P1 | L3 → L4 | R2 TD-005 |
| GAP-009 | matrix_compiler.py にユニットテストなし（1435行・最重要コンパイラ） | quality | P1 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-010 | deliverable_gate.py にユニットテストなし（522行） | quality | P1 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-011 | global_store.py にユニットテストなし（611行・横断データ管理） | quality | P1 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-012 | phase_guard.py にユニットテストなし（366行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-013 | freeze_checker.py にユニットテストなし（351行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-014 | gate_check_generator.py にユニットテストなし（609行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-015 | matrix_advisor.py にユニットテストなし（453行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-016 | recipe_store.py にユニットテストなし（245行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-017 | team_runner.py にユニットテストなし（248行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-018 | merge_settings.py にユニットテストなし（146行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-019 | doc_map_matcher.py にユニットテストなし（111行） | quality | P2 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-020 | builders/ 全14モジュール（2655行）にユニットテストなし | quality | P1 | L3 → L4 | R0 unknowns.no_unit_test |
| GAP-021 | CLI 全体のアーキテクチャ設計書なし | quality | P1 | L2 | R0 unknowns.no_design_doc |
| GAP-022 | Builder System の設計仕様書なし（12モジュール・2655行） | quality | P1 | L2 | R0 unknowns.no_design_doc |
| GAP-023 | Learning Engine の設計仕様書なし（2000行・最大モジュール） | quality | P1 | L2 | R0 unknowns.no_design_doc |
| GAP-024 | DB マイグレーション戦略の文書なし（v1→v4 進行中） | quality | P2 | L2 | R0 unknowns.no_design_doc |
| GAP-025 | 状態機械の遷移仕様書なし（phase_guard.py 内にハードコード） | quality | P2 | L2 | R0 unknowns.no_design_doc |
| GAP-026 | helix-interrupt の IIP/CC 分類ロジック仕様不明（890行） | assumption | P2 | L1 | R0 unknowns.spec_unclear |
| GAP-027 | helix-hook の doc-map トリガー優先度不明 | assumption | P2 | L1 | R0 unknowns.spec_unclear |
| GAP-028 | Builder quality_score の算出基準がビルダーごとに異なり統一基準なし | assumption | P2 | L1 → L2 | R0 unknowns.spec_unclear |
| GAP-029 | recipe の自動昇格閾値が文書化されていない | assumption | P2 | L1 | R0 unknowns.spec_unclear |
| GAP-030 | helix-test と helix-test-debug の重複管理方針不明（1540行 vs 1533行） | assumption | P2 | L2 | R0 unknowns.spec_unclear |
| GAP-031 | fullstack 駆動タイプの L4.5 結合フェーズの CLI 実装が不完全 | scope | P1 | L3 → L4 | R0 unknowns.spec_unclear, R2 TD-008 |
| GAP-032 | RGC（Reverse Gap Closure）が CLI 未実装 | scope | P2 | L3 → L4 | R0 unknowns.spec_unclear, MEMORY.md |
| GAP-033 | Builder System と CLI の接続が設計構想段階 | scope | P1 | L2 → L3 → L4 | R2 TD-007 |
| GAP-034 | fullstack Phase B 結合時の contract CI 自動実行が未実装 | scope | P1 | L3 → L4 | R2 TD-008 |
| GAP-035 | ADR が推定のみ（R2 ADR-001〜010）で正式文書化されていない | quality | P1 | L2 | R2 §4 |
| GAP-036 | yaml_parser.py の制約（アンカー・マージキー・フロー非対応）が明文化されていない | quality | P2 | L2 | R0 unknowns.potential_risks |
| GAP-037 | learning_engine.py が 2000行の巨大ファイル（責務分割候補） | quality | P2 | L2 → L4 | R0 unknowns.potential_risks |
| GAP-038 | Codex 依存のレジリエンス不足（リトライ2回のみ・代替モデルなし） | quality | P2 | L2 | R0 unknowns.potential_risks |
| GAP-039 | helix-debug のテストが helix-test 内に存在しない | quality | P2 | L3 → L4 | R0 unknowns.minimal_test_coverage |
| GAP-040 | helix-verify-all のテストが --help のみ | quality | P2 | L3 → L4 | R0 unknowns.minimal_test_coverage |
| GAP-041 | helix-gate の G1 が valid_values に含まれない（G0.5, G2-G7 のみ） | assumption | P2 | L1 | R1 helix-gate.arguments |
| GAP-042 | helix-reverse R4 の stage_gates が RG3（R3 と同一ゲート） | assumption | P2 | L1 | R1 helix-reverse.stage_gates |
